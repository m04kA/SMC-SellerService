...
# Базовые команды
make build          # Собрать бинарник в bin/smc-sellerservice
...
- **Порт приложения**: 8081 (SMC-UserService использует 8080)
...
⚠️ **Важно**: Это временное решение для MVP. В продакшене будет использоваться полноценная JWT аутентификация через отдельный SMC-AuthService.
...
Метрики автоматически собираются централизованным сервисом **[SMC-Monitoring](https://github.com/m04kA/SMC-Monitoring)**:
...
Готовый дашборд "SMC-SellerService Metrics" включает:
...
SMC-SellerService является частью микросервисной архитектуры **SMC (Slot My Car)**:
...
API Gateway (валидация JWT, маршрутизация)
    ↓
├── SMC-UserService (пользователи, автомобили) - :8080
├── SMC-SellerService (компании, услуги) - :8081
└── SMC-PriceService (цены) - :8082
```
...
Проект разработан для платформы онлайн-записи на автомойку SMC (Slot My Car).
